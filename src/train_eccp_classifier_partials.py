...
if __name__ == "__main__":
    ...
    num_chngs = []
    user_ts = []
    with open(join(outDir, "parts-order-partials-probs.txt"), "w") as out_file:
    ...
    old_xs_test, xs_test, ys_test, num_chngs_test, user_ts_test, next_tkns_test, fixed_xs_test, orig_xs_test, orig_fixes_test, actual_tkns_test = zip(*test_set)
    ...
    if exists(saved_model_file):
        with open(join(outDir, "erules-partials-probs.json"), "r") as fin:
            erules_cnt = json.load(fin)
    else:
        for erls in ys:
            for er in erls:
                erules_cnt[er] += 1
        with open(join(outDir, "erules-partials-probs.json"), "w") as fout:
    ...
    with open(join(outDir, 'models', 'myMultiLabelBinarizer.pkl'), 'wb') as f:
        pickle.dump(mlb, f)
    ...
    if not exists(join(outDir, "test-set-50-popular-partials-probs.txt")):
        ...
        with open(join(outDir, "test-set-50-popular-partials-probs.txt"), "w") as outFile:
            for tokns, eruls, tok_chgs, dur, fixed_tokns, orig_prog, orig_fix, actual_tokns in zip(old_xs_test, ys_test, num_chngs_test, user_ts_test, fixed_xs_test, orig_xs_test, orig_fixes_test, actual_tkns_test):
                popular = "popular"
                if not any(map(lambda yy: yy in top_50_erules_ids, eruls)):
                    popular = "not_popular"
                outFile.write(tokns + " <||> " + " <++> ".join(top_50_erules) + " <||> " + str(tok_chgs) + " <||> " + str(dur) + " <||> " + fixed_tokns + " <||> " + popular+ " <||> " + orig_prog + " <||> " + orig_fix + " <||> " + actual_tokns + "\n")
    ...
    try:
        ...
        with tf.device(gpuToUse):
            ...
            if exists(saved_model_file):
                ...
                history = transformerClfr.fit(x_train, y_train, (x_val, y_val), batch_size=128, epochs=50, verbose=1)
            ...
            avg_num_of_preds = 0
            all_best_preds_lens = []
            for y_pred, y_true in zip(transformerClfr.predict(xs_test), ys_test):
            ...
            print(">> Top 10 predictions acc. (not popular):", progs_top_10_not_pop * 100.0 / not_pop_tests)
            ...
            print(">> Top 50 predictions acc. (not popular):", progs_top_50_not_pop * 100.0 / not_pop_tests)
            print(">> Best predictions accuracy:", progs_best * 100.0 / all_tests)
            print(">> Best predictions acc. (not popular):", progs_best_not_pop * 100.0 / not_pop_tests)
            print(">> Num. of non-popular (not top 50):", not_pop_tests, "(" + str(not_pop_tests * 100.0 / all_tests) + "%)")
            print(">> Avg. Number of best predictions:", avg_num_of_preds / all_tests)
            ...
            # print(">> Top 10 preds acc. (at least one):", progs_top_10_one * 100.0 / all_tests)
            # print(">> Top 20 preds acc. (at least one):", progs_top_20_one * 100.0 / all_tests)
            # print(">> Top 20 preds acc. (at least one) (not popular):", progs_top_20_one_not_pop * 100.0 / not_pop_tests)
            # print(">> Top 50 preds acc. (at least one):", progs_top_50_one * 100.0 / all_tests)
            # print(">> Best preds acc. (at least one):", progs_best_one * 100.0 / all_tests)
            # print(">> Best preds acc. (at least one) (not popular):", progs_best_one_not_pop * 100.0 / not_pop_tests)
            with open(join(outDir, "PartialParsesClassifierStats.txt"), "w") as out_file:
                ...
                out_file.write("Top 10 predictions accuracy: " + str(progs_top_10 * 100.0 / all_tests) + "\n")
                out_file.write("Top 10 predictions acc. (not popular): " + str(progs_top_10_not_pop * 100.0 / not_pop_tests) + "\n")
                out_file.write("Top 20 predictions accuracy: " + str(progs_top_20 * 100.0 / all_tests) + "\n")
                out_file.write("Top 20 predictions acc. (not popular): " + str(progs_top_20_not_pop * 100.0 / not_pop_tests) + "\n")
                out_file.write("Top 50 predictions accuracy: " + str(progs_top_50 * 100.0 / all_tests) + "\n")
                out_file.write("Top 50 predictions acc. (not popular): " + str(progs_top_50_not_pop * 100.0 / not_pop_tests) + "\n")
                out_file.write("Best predictions accuracy: " + str(progs_best * 100.0 / all_tests) + "\n")
                out_file.write("Best predictions acc. (not popular): " + str(progs_best_not_pop * 100.0 / not_pop_tests) + "\n")
                out_file.write("Num. of non-popular (not top 50): " + str(not_pop_tests) + " (" + str(not_pop_tests * 100.0 / all_tests) + "%)" + "\n")
                out_file.write("Avg. Number of best predictions: " + str(avg_num_of_preds / all_tests) + "\n")
                ...
                out_file.write("Top 10 preds acc. (at least one): " + str(progs_top_10_one * 100.0 / all_tests) + "\n")
                out_file.write("Top 20 preds acc. (at least one): " + str(progs_top_20_one * 100.0 / all_tests) + "\n")
                out_file.write("Top 20 preds acc. (at least one) (not popular): " + str(progs_top_20_one_not_pop * 100.0 / not_pop_tests) + "\n")
                out_file.write("Top 50 preds acc. (at least one): " + str(progs_top_50_one * 100.0 / all_tests) + "\n")
                out_file.write("Best preds acc. (at least one): " + str(progs_best_one * 100.0 / all_tests) + "\n")
                out_file.write("Best preds acc. (at least one) (not popular): " + str(progs_best_one_not_pop * 100.0 / not_pop_tests) + "\n")
            if not exists(saved_model_file):
                transformerClfr.save_weights(saved_model_file)

            if do_time_tests:
                # This is not really useful anymore. We have the run_parse scripts for this now.
                ...
                failed = 0
                dataset = [(x, labelize(y_pred, 20), user_t) for x, y_pred, user_t in zip(old_xs_test, transformerClfr.predict(xs_test), user_ts_test)]
                avg_run_time = 0.0
                total_size = 0
                parsed_progs_times = []
                time_gains = []
                    ...
                    while True:
                        try:
                            ...
                            if bruh:
                                ...
                                if (failed + done) % 50 == 0:
                                    print_results(failed, done, parses_bad, avg_run_time, parsed_progs_times, total_size, time_gains, outDir, "trans_cls_results-partials-probs.txt", max_time=120)
                ...
                with open(join(outDir, "test-set-top-20-partials-probs.txt"), "w") as outFile:
                    with open(join(outDir, "test-set-best-partials-probs.txt"), "w") as outFileBest:
                        for tokns, y_pred, tok_chgs, dur, eruls, fixed_tokns, orig_prog, orig_fix, actual_tokns in zip(old_xs_test, transformerClfr.predict(xs_test), num_chngs_test, user_ts_test, ys_test, fixed_xs_test, orig_xs_test, orig_fixes_test, actual_tkns_test):
                            best = list(map(lambda y_tup: y_tup[0], filter(lambda yy: yy[1] > 0.01, enumerate(y_pred))))
                            best_preds = [0] * top_N
                            for i in best:
                                best_preds[i] = 1
                            best_preds = list(mlb.inverse_transform(array(best_preds).reshape(1, num_of_labels))[0])
                            if len(best_preds) > 20:
                                top_20_local = argsort(y_pred)[::-1][:20]
                                top_20_preds_local = [0] * top_N
                                for i in top_20_local:
                                    top_20_preds_local[i] = 1
                                best_preds = list(mlb.inverse_transform(array(top_20_preds_local).reshape(1, num_of_labels))[0])
                            ...
                            if not any(map(lambda yy: yy in top_50_erules_ids, eruls)):
                                popular = "not_popular"
                            outFile.write(tokns + " <||> " + " <++> ".join(labelize(y_pred, 20)) + " <||> " + str(tok_chgs) + " <||> " + str(dur) + " <||> " + fixed_tokns + " <||> " + popular + " <||> " + orig_prog + " <||> " + orig_fix + " <||> " + actual_tokns + "\n")
                            outFileBest.write(tokns + " <||> " + " <++> ".join(best_preds) + " <||> " + str(tok_chgs) + " <||> " + str(dur) + " <||> " + fixed_tokns + " <||> " + popular + " <||> " + orig_prog + " <||> " + orig_fix + " <||> " + actual_tokns + "\n")
